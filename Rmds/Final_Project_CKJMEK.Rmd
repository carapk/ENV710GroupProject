...
# load the data
lakes_raw <- read.csv("./Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv",
                      stringsAsFactors = TRUE) 
do_raw <- read.csv("Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv",
                   stringsAsFactors = TRUE)
...
#select desired columns for analysis based on which lakes have the most data, 
#create our binary indicator variables
lakes_processed <- lakes_raw %>%
  select(lakename, sampledate, depth, tn_ug, tp_ug, nh34, no23, po4) %>%
  na.omit() %>%
  filter(lakename %in% c("Paul Lake", "West Long Lake", "Peter Lake", 
                         "East Long Lake")) %>%
  ...
  mutate(year = factor(year(sampledate)),
         ...
         season = factor(ifelse(month == 5,
                                "spring", 
                                ifelse(month == 8,  "summer", NA)))) %>%
...
#join with dissolved oxygen data
lakes_processed <- left_join(lakes_processed, 
                             select(do_raw, lakename, sampledate, 
                                    depth, dissolvedOxygen), 
...
#only selecting for spring and summer
...
#plot results of model one, DO v TP by season
...
df <- nrow(lakes_processed) - length(fixef(fit))

# Calculate the two-tailed p-value
p_value_tp_ug <- 2 * pt(abs(t_value_tp_ug), df = df, lower.tail = FALSE)
