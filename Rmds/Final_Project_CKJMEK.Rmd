...
```{r Data Exploration, warning=FALSE, include=FALSE}
...
mean_DO <- mean(na.omit(lakes_processed$dissolvedOxygen))
median_DO <- median(na.omit(lakes_processed$dissolvedOxygen))
range_DO <- range(na.omit(lakes_processed$dissolvedOxygen))
```
...
| $\beta_2$             | -0.067 (mg/L)/$\mu$g | Expected change in DO for each unit increase in TP in the spring                   |
...
```{r seasonal model holdout and RMSE calculation, include = FALSE, message=FALSE, warning=FALSE}
...
RMSE_seasonal <- sqrt(mean((two_season_testing$dissolvedOxygen - two_season_testing$do_predicted)^2))

print(RMSE_seasonal)

```

```{r seasonal model plot, fig.cap="Dissolved Oxygen vs Total Phosphorus by Season", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r depth model holdout and RMSE calculation, include = FALSE, message=FALSE, warning=FALSE}
...
RMSE_depth <- sqrt(mean((jenn_testing$dissolvedOxygen - jenn_testing$do_predicted)^2, na.rm = TRUE))
...
```{r depth plot, fig.cap = "Dissolved Oxygen vs Total Phosphorus by Water Depth", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r hierarchical model holdout and RMSE calculation, include = FALSE, message=FALSE, warning=FALSE}
...
RMSE_hierarchical <- sqrt(mean((lakes_testing$dissolvedOxygen - lakes_testing$do_predicted)^2, na.rm = TRUE))

print(RMSE_hierarchical)

```

```{r hierarchical plot, fig.cap = "Dissolved Oxygen vs Total Phosphorus by Lake", echo=FALSE, message=FALSE, warning=FALSE}
...
In terms of model performance, the Root Mean Squared Error (RMSE) score of 3.32 indicates a fairly high degree of error when comparing the fitted values to the observed data. This suggests that while the model may provides useful estimates, there is muhch room for improvement in capturing the variability in dissolved oxygen explained by total phosphorus across different lakes. Further refinements or alternative modeling approaches may be warranted to enhance predictive accuracy and better account for the complexities in the data.

# Analysis and Conclusion

Model Performance: To assess the effectiveness of our models in predicting dissolved oxygen (DO) levels based on total phosphorus (TP) concentrations, we evaluated three different models: Depth Model, Seasonal Model, and Hierarchical Model by Lakename. Each model's performance was measured using the Root Mean Squared Error (RMSE) metric, providing insights into the accuracy of predictions compared to observed data.

Table: Model RMSE Comparission
...
Emma wrote the intro, methods, model 1 results, and fit model 1. Jenn wrangled the datasets for analysis, completed the testing/training of all models, wrote model 2 results, and fit model 2. Cara wrote the analysis and conclusion, model 3 results, and fit model 3. All group members contributed in fine-tuning the final submission.
